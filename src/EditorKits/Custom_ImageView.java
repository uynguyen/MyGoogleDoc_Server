...
/*
This code was gathered from the OpenJDK 7
    http://download.java.net/openjdk/jdk7/
    Copy file:  openjdk/jdk/src/share/classes/javax/swing/text/html/ImageView.java
    Change all "synchronized(ImageView.this)"  to  "synchronized(RITB64_ImageView.this)"
Related websites:
    http://stackoverflow.com/questions/20943833/how-to-render-a-internal-image-on-a-swing-component-using-html
    http://stackoverflow.com/questions/2735808/base64-image-into-jtextpane
    http://www.javaworld.com/article/2077472/core-java/java-tip-109--display-images-using-jeditorpane.html
*/



public class Custom_ImageView extends View {
    ...
    private class ImageHandler implements ImageObserver {
        ...
        public boolean imageUpdate(Image img, int flags, int x, int y,
            ...
            if ((flags & (ABORT|ERROR)) != 0) {
                repaint(0);
                synchronized(Custom_ImageView.this) {
                    if (image == img) {
    ...
    private String getBASE64Image() {
        ...
        return src.substring(src.indexOf("base64,") + "base64,".length());
    } // End getBASE64Image()...
  
} // End RITB64_ImageView()...
...
/*  Unused code...
    private final static int DEFAULT_IMAGE_WIDTH = 32;    
    private int getImageWidth() {
        int iWidth = DEFAULT_IMAGE_WIDTH;   // Default value of 32 pixes...
        int origWidth = DEFAULT_IMAGE_WIDTH;
        if(image != null) {
            origWidth = image.getWidth(imageObserver);
            //System.out.println("[DEBUG] getImageWidth() - iWidth:  " + iWidth);
            String src = ((String) getElement().getAttributes().getAttribute(HTML.Attribute.WIDTH)).toLowerCase();
            //System.out.println("[DEBUG] getImageWidth() - HTML Attributes:  " + src);
            if(src == null || src.equals("")) { // If there is no width field, then return the original image width...
                return origWidth;
            }
            try {
                // See if we have a width specified in the <IMG> tag...
                if(src.contains("%")) { // width="10%"
                    String s = src.replaceAll("\\%", "");
                    double widthPercentage = Double.parseDouble(s) / 100;
                    iWidth = (int) ((double)origWidth * widthPercentage);
                } else if(src.contains("px")) { // width="200px"
                    String s = src.replaceAll("px", "");
                    iWidth = Integer.parseInt(s);
                } else { // width="200"                    
                    iWidth = Integer.parseInt(src);
                }
                //System.out.println("[DEBUG] getImageWidth() - iWidth(Orig -> New):  " + origWidth + " -> " + iWidth);
                return iWidth;
            } catch(Exception e) {
                System.out.println("[ERROR] getImageWidth() \n\t" + e);
                return DEFAULT_IMAGE_WIDTH;
            } // End catch(e)...
        } // End if(image != null)...
        // Should never reach here.  If it does, it will return the default value of 32...
        return DEFAULT_IMAGE_WIDTH;
    } // End getImageWidth()...
    ...
    private final static int DEFAULT_IMAGE_HEIGHT = 32;
    private int getImageHeight() {
        int iHeight = DEFAULT_IMAGE_HEIGHT; // Default value of 32 pixes...
        int origHeight = DEFAULT_IMAGE_HEIGHT;
        if(image != null) {
            origHeight = borderSize;
            //System.out.println("[DEBUG] getImageHeight() - iHeight:  " + iHeight);
            String src = ((String) getElement().getAttributes().getAttribute(HTML.Attribute.HEIGHT)).toLowerCase();
            //System.out.println("[DEBUG] getImageHeight() - HTML Attributes:  " + src);
            if(src == null || src.equals("")) { // If there is no width field, then return the original image height...
                return origHeight;
            }
            try {
                // See if we have a height specified in the <IMG> tag...
                if(src.contains("%")) { // height="10%"
                    String s = src.replaceAll("\\%", "");
                    double widthPercentage = Double.parseDouble(s) / 100;
                    iHeight = (int) ((double)origHeight * widthPercentage);
                } else if(src.contains("px")) { // height="200px"
                    String s = src.replaceAll("px", "");
                    iHeight = Integer.parseInt(s);
                } else { // height="200"                    
                    iHeight = Integer.parseInt(src);
                }
                //System.out.println("[DEBUG] getImageHeight() - iHeight(Orig -> New):  " + origHeight + " -> " + iHeight);
                return iHeight;
            } catch(Exception e) {
                System.out.println("[ERROR] getImageHeight() \n\t" + e);
    ...
    private final static int DEFAULT_IMAGE_BORDER_WIDTH = 0;
    private int getImageBorderWidth() {
        int iBorderWidth = DEFAULT_IMAGE_BORDER_WIDTH; // Default value of 32 pixes...
        int origBorderWidth = DEFAULT_IMAGE_BORDER_WIDTH;
        if(image != null) {
            origBorderWidth = image.getHeight(imageObserver);
            //System.out.println("[DEBUG] getImageBorderWidth() - iHeight:  " + iHeight);
            String src = ((String) getElement().getAttributes().getAttribute(HTML.Attribute.BORDER)).toLowerCase();
            System.out.println("[DEBUG] getImageBorderWidth() - HTML Attributes:  " + src);
            if(src == null || src.equals("")) { // If there is no width field, then return the original image height...
                return origBorderWidth;
            }
            try {
                // See if we have a border width specified in the <IMG> tag...
                // border="0"
                iBorderWidth = Integer.parseInt(src);
                System.out.println("[DEBUG] getImageBorderWidth() - iHeight(Orig -> New):  " + origBorderWidth + " -> " + iBorderWidth);
                return iBorderWidth;
            } catch(Exception e) {
                System.out.println("[ERROR] getImageBorderWidth() \n\t" + e);
                return DEFAULT_IMAGE_BORDER_WIDTH;
            } // End catch(e)...
        } // End if(image != null)...
        // Should never reach here.  If it does, it will return the default value of 32...
        return DEFAULT_IMAGE_BORDER_WIDTH;
    } // End getImageBorderWidth()...
// Original paint function...
    /**
     * Paints the View.
     *
     * @param g the rendering surface to use
    ...
    */    
    ...
    public void paint(Graphics g, Shape a) {
        ...
        fBounds.setBounds(rect);
        // Get the width & height from the HTML Tag...
        int imageWidth = getImageWidth();
        int imageHeight = getImageHeight();
        //System.out.println("[DEBUG] Dimension(w x h):  " + image.getWidth(imageObserver) + " x " + image.getHeight(imageObserver));
        // Get and draw the border...
        int borderWidth = getImageBorderWidth();
        // Seems like the 
        int newX = fBounds.x;
        int newY = fBounds.y;
        //paintBorder(g, rect); // This doesn't work...
        if(borderWidth > 0) {
            // Adjust the position and size according to the border width...
            newX += borderWidth;
            newY += borderWidth;
            int newWidth = imageWidth + borderWidth;
            int newHeight = imageHeight + borderWidth;
            //int newWidth = imageWidth + (2*borderWidth);
            //int newHeight = imageHeight + (2*borderWidth);
            g.drawRect(fBounds.x, fBounds.y, newWidth, newHeight);
            //g.drawRect(fBounds.x, fBounds.y, imageWidth, imageHeight);
        } // End borderWidth()...
                
        // Load the image, from BufferedImage...
        loadImage();
        // Draw the image...
        g.drawImage(image, newX, newY, imageWidth, imageHeight, imageObserver); // Draw the BufferedImage...
    } // End paintComponent_BufferedImage()...
    */
    ...
    /** /  My code which seems to work fine ... 
    private void loadImage() {
        String b64 = getBASE64Image();        
        BufferedImage newBufferedImage = null;
        try (ByteArrayInputStream bais = new ByteArrayInputStream(DatatypeConverter.parseBase64Binary(b64))) {
        ...
        image = newImage; 
        //System.out.println("[DEBUG] X,Y-Border: " + width + "," + height + "-" + borderSize);
    } // End loadImage()...
    */